# *****************************************************************************
#
# Script: cSTM_sick-sicker_time_exercise_solutions.R
#
...
df_M_SoC <- data.frame(Cycle = 0:n_cycles, m_M_SoC, check.names = F)

# Convert to long format
df_M_SoC_long <- tidyr::gather(df_M_SoC, key = `Health State`, value, 
                               2:ncol(df_M_SoC))

# df_M_SoC_long factor levels to health states
df_M_SoC_long$`Health State` <- factor(
  df_M_SoC_long$`Health State`,
  levels = c("H", "S1", "S2", "D"),              # existing values
  labels = c("Healthy", "Sick", "Sicker", "Dead") # new labels
)

#Plot 
ggplot(df_M_SoC_long, aes(x = Cycle, y = value, color = `Health State`, 
...
df_strategies             <- rbind(df_M_SoC_long, df_M_strAB_long)
...
# Interpretation:
# - The proportion in the Dead state increases more quickly at older ages due to
#   age-dependent mortality.
# - Strategy AB slows progression to the Sicker state and keeps more people in the
#   Healthy/Sick states for longer compared to SoC.
# - Eventually, both strategies converge to 100% in the Dead state by very old ages.
...
# (We incorporate cycle_length directly when defining the reward vectors.)
...
v_u_strAB <- c(H  = u_H,
               S1 = u_trtAB,  # improved utility with treatment AB in S1
               S2 = u_S2,
               D  = u_D) * cycle_length
...
v_c_strAB <- c(H  = c_H,
               S1 = c_S1 + c_trtAB,  # add treatment cost in S1
               S2 = c_S2 + c_trtAB,  # add treatment cost in S2
               D  = c_D) * cycle_length
...
# Store the vectors of state costs for each strategy in a list 
...
for (i in 1:n_str) {
  # Cycle-specific QALYs and costs
  v_qaly_str <- l_m_M[[i]] %*% l_u[[i]]
  v_cost_str <- l_m_M[[i]] %*% l_c[[i]]
  
  # Apply discounting and within-cycle correction
...
knitr::kable(results_df,
             caption = "Total Expected Outcomes by Strategy (Age-Dependent Sick-Sicker Model)")

# Interpretation of results:
# - Total QALYs are generally lower than in a time-independent model because
#   mortality increases with age and shortens life expectancy.
# - Strategy AB yields higher QALYs than SoC due to slower progression and
#   higher utility in the Sick state.
# - Strategy AB is more costly than SoC because of the additional treatment cost
#   applied to both S1 and S2 states.
...
knitr::kable(table_cea,
             caption = "Cost-Effectiveness Analysis Results (Age-Dependent Model)")

# Interpretation of ICER results:
# - The ICER for Strategy AB vs SoC reflects the additional cost per additional
#   QALY gained with AB.
# - Because age-dependent mortality reduces remaining life expectancy, the ICER
#   may differ from the simpler time-independent model (often higher).
# - Whether AB is considered cost-effective depends on the willingness-to-pay
#   threshold (e.g., $50,000–$100,000 per QALY).
...
# The formatted table was already created as `table_cea` above.
# You can export it if desired using write.csv() or similar functions.

# ADDITIONAL INTERPRETATION:
# - The CEA table summarizes total costs, QALYs, incremental outcomes, and ICERs.
# - It clearly shows which strategy is non-dominated and where Strategy AB lies
...
plot(df_cea, label = "all", txtsize = 12) +
  ...
  labs(title = "Cost-Effectiveness Frontier (Age-Dependent Sick-Sicker Model)",
       x = "Total QALYs",
       y = "Total Costs ($)") +
  theme_minimal(base_size = 12) +
  theme(legend.position = c(0.8, 0.3))

# Interpretation of CEA frontier:
# - SoC should appear as the least costly (and least effective) option.
# - Strategy AB lies to the upper-right of SoC, reflecting higher cost and higher QALYs.
# - The line connecting non-dominated strategies represents the cost-effectiveness
#   frontier in this age-dependent setting.
